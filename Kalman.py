...
class Kalman:
    ...
    # helper functions to set Q, R, L attributes of Kalman object
    def setQ(Q):
        self.Q = Q
    ...
    def predictionNoCamera(u, dt_kalman):
    ...
    def distToCone(x_k, cone):
        return sqrt(math.pow(x_k[0] - cone[0], 2) + math.pow(x_k[1] - cone[1], 2))
    ...
    def sendToArduino(x_k, x, y):
        pwm_angle = np.array([DEFAULT_PWM, 0])
        cone = np.array([x, y])
        if (distToCone(x_k, cone) <= MIN_DIST):
            pwm_angle[0] = 0
