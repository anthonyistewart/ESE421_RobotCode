...
Matrix<3, 1> KalmanFilter::prediction(Matrix<2, 1> u, Matrix<3, 1> z_k, float dt) {
  ...
  this->x_hat_prime(2) = correctAngle(this->x_hat_last(2) + dt * (u(0) / this->L) * u(1)); //Calculate psi
  this->x_hat_prime(0) = this->x_hat_last(0) + dt * u(0) * cos(this->x_hat_last(2));
  this->x_hat_prime(1) = this->x_hat_last(1) + dt * u(0) * sin(this->x_hat_last(2));
...
double KalmanFilter::correctAngle(double angle) {
